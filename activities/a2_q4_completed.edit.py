...
def introduction(mo):
    mo.md(r"""
    # A2 Q4 · Completed reference notebook
    ...
    The residual and parity loops are completed. The full dataset comparison
    is shown in Q4.4, and Q4.5 starts with the 100-cluster fit.
    """)
    return
...
@app.cell(hide_code=True)
def all_fits(clusters, energy_errors, fit_LJ, mo, residual_ok):
    mo.stop(not residual_ok)
    fit_rows = []
    for count in [10, 20, 40, 60, 80, 100]:
        result = fit_LJ(clusters[:count])
        if not result.success:
            raise RuntimeError(result.message)
        sigma, epsilon = result.x
        _, mae = energy_errors(clusters[:count], sigma, epsilon)
        fit_rows.append({"N": count, "sigma (Å)": float(sigma),
                         "epsilon (eV)": float(epsilon), "MAE (meV/atom)": mae})
    best_sigma = fit_rows[-1]["sigma (Å)"]
    best_epsilon = fit_rows[-1]["epsilon (eV)"]
    mo.vstack([mo.md("## Q4.4 · Results for all six dataset sizes"),
               mo.ui.table(fit_rows, selection=None)])
    return best_epsilon, best_sigma, fit_rows
...
def extrapolation_instructions(mo):
    ...
    from fitting. The inputs start with the 100-cluster fit. For each cluster, the supplied calculation uses
    `E_LJ = calculate_cluster_energy(coords, epsilon, sigma)`.
...
@app.cell(hide_code=True)
def extrapolation_inputs(best_epsilon, best_sigma, mo):
    chosen_sigma = mo.ui.number(value=best_sigma, step=1e-8, label="Fitted sigma (Å)")
    chosen_epsilon = mo.ui.number(value=best_epsilon, step=1e-10, label="Fitted epsilon (eV)")
