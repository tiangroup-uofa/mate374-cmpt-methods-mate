...
def fixed_algorithm(np):
    ...
    def student_map(x):
        # Edit this function in the notebook to test your own rearrangement.
    ...
    return fixed_map, fixed_residual, fixed_roots, trace_fixed


@app.cell(hide_code=True)
def fixed_controls(mo):
    fixed_choice = mo.ui.dropdown(
        ["Constant c = λ", "c = −x/2", "c = 1/(3x)", "Logarithm", "Positive square root", "Negative square root", "Newton", "Your map"],
...
def fixed_plot(
    ...
    def iteration_equations(choice, relaxation):
        formulas = {
            ...
            "Newton": r"x-\frac{3x^2-x+e^{2x}-2}{6x-1+2e^{2x}}",
        }
        notes = {
            ...
            "Newton": r"The denominator must be nonzero.",
        }
        if choice == "Constant c = λ":
        ...
        equation = (
            "### Selected iteration: **" + choice + "**\n\n"
            "The residual is\n\n"
            "$$F(x)=3x^2-x+e^{2x}-2$$\n\n"
            "The update is\n\n"
            "$$x_{k+1}=g(x_k),\\qquad g(x)=" + formula + "$$"
        )
        if note:
            equation += "\n\n" + note
        return mo.md(equation)
    ...
    mo.vstack([
        fixed_equations,
        fixed_figure,
        mo.md(f"**Full run:** {fixed_status}. Last x = **{fixed_sequence[-1]:.8g}**; |F| = **{fixed_final_residual:.3g}**.\n\n"
