...
def solve(force, np, springs):
    ...
    F = np.asarray(force.value, dtype=float).reshape(-1)
...
def chain_widget(anywidget, traitlets):
    class SpringChain(anywidget.AnyWidget):
        ...
        function render({ model, el }) {
          ...
          draw();
          const names = ["springs", "f", "u", "k_max"];
          for (const name of names) model.on(`change:${name}`, draw);
          return () => {
            for (const name of names) model.off(`change:${name}`, draw);
          };
        }
