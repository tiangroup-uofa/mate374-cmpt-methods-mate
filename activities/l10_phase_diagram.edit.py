...
def introduction(mo):
    ...
    **Live demo notebook.** The four cells marked **Live step** are typed in
    class. The collapsed cells supply the data, the well limits, the plots, and
    the checks, and each check turns green once its step works.
...
@app.cell
def live_fit():
    # Live step 1: define vdw_pressure(state, a, b), then fit a_fit, b_fit with curve_fit.
    vdw_pressure = None
...
@app.cell
def live_minimize():
    # Live step 2: define free_energy(v, T, P, a, b) in J/mol and
    # phase_volumes(T, P, a, b), which returns (v_liq, v_gas).
    free_energy = None
    phase_volumes = None
...
@app.cell
def live_root():
    # Live step 3: define delta_G(P, T, a, b) and solve_pressure(T, a, b) with root_scalar.
    delta_G = None
    solve_pressure = None
...
@app.cell
def live_boundary():
    # Live step 4: five saturation pressures from 250 to 295 K, then a PCHIP boundary.
