...
def introduction(mo):
    ...
    **Student live-coding notebook.** Complete the four cells marked **Live step**
    in order. Replace each cell's `None` placeholders with your functions and
    calculations. Figures and checks appear as you complete each stage.
    The collapsed cells supply measurements, search intervals, and the
    switchable interpolation helper.
...
@app.cell
def live_fit():
    # Live step 1: define vdw_pressure(state, a, b), unpacking state as (T, v).
    # Fit a_fit and b_fit to (T_fit, v_fit), P_fit with p0=(3.6, 0.043).
    vdw_pressure = None
...
@app.cell
def live_minimize():
    # Live step 2: define free_energy(v, T, P, a, b) in J/mol.
    # Define find_state_volume(T, P, a, b, bounds) for one bounded minimization.
    # In phase_volumes(T, P, a, b), obtain v1, v2 from turning_volumes.
    # Search (1.0001*b, v1) and (v2, 5*R*T/P), then return v_liq, v_gas.
    free_energy = None
    find_state_volume = None
    phase_volumes = None
...
@app.cell
def live_root():
    # Live step 3: define delta_G(P, T, a, b), recalculating both phase volumes.
    # Define solve_pressure(T, a, b) using find_pressure_bracket and method="bisect".
    # Check result.converged, then return result.root. Later, try "brentq".
    delta_G = None
    solve_pressure = None
...
@app.cell
def live_boundary():
    # Live step 4: choose five temperatures from 250 to 295 K with np.linspace.
    # Calculate P_grid with solve_pressure(T, a_fit, b_fit) at each temperature.
    # Use interpolate_boundary(T_grid, P_grid, method="linear").
    # Then switch method to "cubic" or "pchip" and compare the 275 K check.
    T_grid, P_grid, boundary = None, None, None
