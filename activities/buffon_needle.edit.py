...
def _(mo):
    ...
    Choose your inputs, then press **Simulate**. Changing a slider does not
...
@app.cell(hide_code=True)
def _(mo):
    seed_text = mo.ui.text(
        value="374",
        label="Random seed (replace 374 with your student ID)",
    ...
    mo.vstack(
        [seed_text, length_ratio, number_of_needles, simulate],
        align="start",
        gap=1,
    )
    return length_ratio, number_of_needles, seed_text, simulate
...
def _(
    ...
    if not simulate.value:
        ...
        seed_value = seed_text.value.strip()

        if not seed_value.isdigit():
            output = mo.callout(
                mo.md(
                    "The random seed must contain digits only. "
                    "Correct the seed and press **Simulate** again."
                ),
                kind="danger",
            )
        else:
            ...
            display_count = min(sample_count, 180)
