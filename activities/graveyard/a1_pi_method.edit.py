...
def _(mo):
    ...
    Find a numerical method using an AI tool, a search engine, or the
    literature. Explain its equation and what its effort parameter $N$
...
def _(mo):
    ...
    - If $E\approx C N^{-p}$ over a useful range, estimate $p$ from the
      negative slope of the log–log plot. Do not fit points at the precision limit.
    - A rapidly converging method need not follow a power law. A random
      method need not improve at every point; repeat it with several seeds
      before claiming a trend.
    - Explain the trend you actually observe, including plateaus or failures.
