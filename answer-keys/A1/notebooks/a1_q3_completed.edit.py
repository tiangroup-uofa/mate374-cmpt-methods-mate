...
def _(mo):
    mo.md(r"""
    # A1 Q3 · Completed reference: Leibniz series

    This reference uses the
    [Leibniz series](https://en.wikipedia.org/wiki/Leibniz_formula_for_%CF%80),
    $\pi_N=4\sum_{k=0}^{N-1}(-1)^k/(2k+1)$. Here $N$ counts retained terms.

    `estimate_pi` is completed. The supplied code compares its output
