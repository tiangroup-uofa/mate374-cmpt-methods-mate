...
Notice the average time per throw **decreases** as $N$ grows: from $2.0\ \mu$s at $10^3$ down to $0.65\ \mu$s at $10^6$. Part of this is a fixed startup cost being spread over more throws, but the drop is too large to explain by overhead alone — the *entire* run at $N = 10^3$ finishes in 2 ms, so the startup overhead is at most a few milliseconds.

### The simple runtime model

A first model for the total time is

$$
t_{\mathrm{total}} \approx N\,\langle t_{\mathrm{step}}\rangle + t_{\mathrm{overhead}},
$$

where $\langle t_{\mathrm{step}}\rangle$ is a constant per-throw cost and $t_{\mathrm{overhead}}$ is a fixed startup cost (function call, loop setup, final division). The random-number generator is initialized before the timer starts; imports and plots are also outside it.

If this model held exactly, the ratio $t_{\mathrm{total}} / N$ would converge to $\langle t_{\mathrm{step}}\rangle$ from above as $N$ grows. The table shows the right trend, but the per-throw time drops by a factor of three from $10^3$ to $10^6$ — more than a few-millisecond overhead can account for.

::: {.callout-important}
## When the model does not fit

A two-point fit using $N = 10^5$ and $10^6$ would give $t_{\mathrm{overhead}} \approx 29$ ms. But the entire $N = 10^3$ run takes only 2 ms, so a 29 ms overhead is impossible. The per-step cost itself is not constant: the WASM engine optimizes the hot loop during execution, so later iterations run faster than early ones. At larger $N$ a bigger fraction of the loop benefits from this warmup, lowering the average.

The simple linear model is still a useful approximation at large $N$, where the per-throw time has nearly stabilized — but we should not trust it to extrapolate backward to small $N$.
:::

### Extrapolating to $10^9$ throws

Since the per-throw time is still decreasing at $10^6$, we use the value at our largest measured $N$ as a conservative upper bound:

$$
t(10^9) \lesssim 10^9 \times 0.65\ \mu\text{s} \approx 650\ \text{s} \approx \mathbf{11\ \text{min}}.
$$

The actual time is likely somewhat less, because the per-throw cost may continue to decrease slightly at larger $N$. But the order of magnitude — minutes, not seconds or hours — is reliable enough for planning whether a computation is practical.

Eleven minutes in a browser notebook is feasible but slow, and this is only the Buffon experiment. A grain-growth simulation on a $1024\times1024$ grid or a molecular dynamics calculation performs far more work per step.

*Check:* what would make even this upper-bound extrapolation unreliable? Consider competing programs, processor thermal throttling, and changes in memory access patterns at very large $N$.
