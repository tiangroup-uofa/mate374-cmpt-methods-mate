...
where $\langle t_{\mathrm{step}}\rangle$ is the marginal cost of one additional throw and $t_{\mathrm{overhead}}$ is fixed startup cost (function call, loop setup, final division). The random-number generator is initialized before the timer starts; imports and plots are also outside it. A different timed block would include different costs.
