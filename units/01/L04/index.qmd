...
Notice the average time per throw **decreases** as $N$ grows. This is because a fixed startup cost is spread over more throws. At large $N$, the per-throw average approaches a constant.
...
where $\langle t_{\mathrm{step}}\rangle$ is the marginal cost of one additional throw and $t_{\mathrm{overhead}}$ is fixed startup cost (function call, random-generator initialization). Imports and plots are outside our timer; a different timed block would include different costs.

We estimate both parameters from the two largest measurements, where the overhead is a smaller fraction of total time:

$$
\langle t_{\mathrm{step}}\rangle \approx \frac{t(10^6) - t(10^5)}{10^6 - 10^5} = \frac{0.6530 - 0.0910}{9\times10^5} \approx 6.2\times10^{-7}\ \text{s} \approx 0.62\ \mu\text{s}.
$$

The overhead follows from either measurement:

$$
t_{\mathrm{overhead}} \approx t(10^5) - 10^5\times\langle t_{\mathrm{step}}\rangle \approx 0.091 - 0.062 \approx 0.029\ \text{s}.
$$

::: {.callout-tip}
## Why use the two largest $N$?

At small $N$, the overhead dominates and the per-throw average is inflated ($2.0\ \mu$s at $10^3$ vs. $0.65\ \mu$s at $10^6$). Using the largest $N$ values isolates the per-step cost from startup noise.
:::

### Extrapolating to $10^9$ throws

With our fitted model, a billion throws would take:

$$
t(10^9) \approx 10^9 \times 6.2\times10^{-7} + 0.03 \approx 620\ \text{s} \approx \mathbf{10\ \text{min}}.
$$

At this scale, the overhead ($0.03$ s) is negligible — the total time is dominated by $N\,\langle t_{\mathrm{step}}\rangle$. Ten minutes in a browser notebook is feasible but slow, and this is only the Buffon experiment. A grain-growth simulation on a $1024\times1024$ grid or a molecular dynamics calculation performs far more work per step.

*Check:* is time per throw nearly constant at the larger sizes? What would make the extrapolation unreliable? Consider competing programs, processor temperature, and changes in memory access patterns.
