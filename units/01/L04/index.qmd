...
If this model held exactly, $t_{\mathrm{total}}/N$ would converge to $\langle t_{\mathrm{step}}\rangle$ as $N$ grows. What do the measurements show?

The per-throw average first **drops** — from $2.0\ \mu$s at $10^3$ to $0.65\ \mu$s at $10^6$ — then **rises** slightly — to $0.79\ \mu$s at $10^8$. Neither trend matches a constant $\langle t_{\mathrm{step}}\rangle$:

- **Small $N$:** the per-throw time is inflated, but the total time at $N = 10^3$ is only 2 ms, so the startup overhead is at most a few milliseconds — far too small to explain a $3\times$ drop to $0.65\ \mu$s. The WASM engine optimizes the hot loop during execution, so later iterations run faster and longer runs benefit more from this warmup.
- **Large $N$:** the per-throw time rises from $0.65$ to $0.79\ \mu$s. Possible causes include processor thermal throttling, browser garbage-collection pauses, or other background work accumulating over a longer run.

### Where the linear model does work

Despite the non-constant per-throw time, the scaling at large $N$ is close to linear. Check the ratio of total time when $N$ increases tenfold:

| Step | $t(10N) / t(N)$ |
|---|---:|
| $10^5 \to 10^6$ | $0.653 / 0.091 = 7.2$ |
| $10^6 \to 10^7$ | $7.156 / 0.653 = 11.0$ |
| $10^7 \to 10^8$ | $78.63 / 7.156 = 11.0$ |

For $N \ge 10^6$ the ratio has settled near $\approx 11$: each tenfold increase in throws costs about $11\times$ the time. This is close to the $10\times$ expected from pure linear scaling, with a small extra factor from the rising per-step cost.

### Extrapolating to $10^9$ throws

We can extrapolate from the largest measured point using the stabilized ratio:

$$
t(10^9) \approx 78.6\ \text{s} \times 11 \approx 860\ \text{s} \approx \mathbf{14\ \text{min}}.
$$

Alternatively, using the per-throw time at $10^8$ directly: $10^9 \times 0.79\ \mu\text{s} \approx 790$ s $\approx 13$ min. Both estimates agree on the order of magnitude.
...
*Check:* the ratio extrapolation assumed the $\approx 11\times$ factor continues to $10^9$. What could change it? Consider thermal throttling over a 14-minute run, memory-access patterns, and competition from other browser tabs.
