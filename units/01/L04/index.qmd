...
5. **Connect** these acceleration strategies to large materials simulations, including 2D Ising and Potts microstructure models.
...
$$
N \approx \left(\frac{2.37}{5 \times 10^{-5}}\right)^2 \sim 10^9\ \text{throws}.
$$
...
The experiment from L02 can be summarized with the function below. `throw_needle(L, D)` simulates one needle and returns 1 if it crosses a line, or 0 otherwise. We can leave the details of that function aside for now.
...
Each needle throw is **independent** of the others. In this sequential implementation, the time spent on each throw adds to the total. Independence will also allow us to divide the work later. For now, we first ask: how long does one throw take?
...
For a simple timing measurement, use Python's `time.perf_counter()`. For our Buffon experiment:
...
We use the same crossing test as L02, setting the needle length $L$ and line spacing $D$ to 1. The function below implements `throw_needle(L, D)` for $L\leq D$. By symmetry, we only need to sample the distance to the nearest line in $[0,D/2]$ and the angle in $[0,\pi/2]$.
...
The random-number generator is initialized outside both functions. Each call to `throw_needle` returns either 0 or 1, and `buffon_experiment` adds these results to obtain the crossing count.
...
Using this browser-based Python notebook, I recorded the elapsed time for several values of $N$.
...
Fourteen minutes in a browser notebook is feasible, but it is slow and certainly not optimized. And this is only the Buffon experiment. A grain-growth simulation on a $1024\times1024$ grid or a molecular dynamics calculation performs far more work at each step.

## Why are Python loops slow?

Python is excellent for prototyping and scientific computing. However, each pass through a Python loop involves more than the arithmetic we wrote. The interpreter reads bytecode, looks up and handles Python objects, dispatches function calls, updates the loop counter, and then moves to the next iteration.

![Why a short Python loop can spend more time on interpreter bookkeeping than on arithmetic. The repeated chores are shown conceptually; this is not a measured breakdown of the Buffon program.](../../../assets/L04-python-loop-overhead.png){#fig-python-loop-overhead fig-alt="A comic-style diagram follows one Python loop iteration through bytecode reading, name lookup, function dispatch, dynamic type handling, calculation, and object updates. It emphasizes that the small arithmetic operation is accompanied by interpreter work on every iteration." width="92%"}

A single needle throw contains only a small amount of numerical work, so the repeated interpreter bookkeeping can become a substantial part of the runtime. We timed the whole function rather than profiling individual operations, so the illustration does not claim a percentage for each task.
...
This is the catch behind our earlier advice to "just use Python." Python makes an algorithm easy to express, but a pure Python loop may be too slow for a realistic simulation: tens of thousands of diffusion time steps, grain growth on a $2048\times2048$ grid, or millions of interatomic-force calculations. In such cases, the cost of one step determines whether the full calculation is practical.

For the rest of the lecture, we will keep the Buffon model fixed and change only its implementation. This lets us compare several ways to **make the program faster** without changing the numerical experiment. You do not need to memorize every technique; return to these examples when your own program needs a speedup.
