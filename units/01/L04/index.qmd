...
On a browser-based Python notebook like this, I recorded the elapsed time for several values of $N$:
...
On a log–log plot, $t \propto N$ is a straight line with slope 1. The measured points track this reference line closely: the scaling is approximately linear with the per-throw run time around $0.65$–$0.79\ \mu$s. The slight curvature at small $N$ reflects engine warmup; the mild steepening at large $N$ may reflect thermal throttling or garbage-collection pauses.

By fitting a straight line through the large-$N$ data (we will learn linear fitting later in this course) and extrapolating to $10^9$ throws, the estimated run time is approximately 860 seconds, or about 14 minutes.
