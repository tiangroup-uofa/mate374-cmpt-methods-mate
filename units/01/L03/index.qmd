...
::: {.callout-important}
A tiny representation error can matter when multiplied by a large count. For a specified format, rounding mode, and sequence of operations, round-off is **deterministic**, not random noise. We can inspect individual stored values exactly, although predicting how their errors propagate through a long calculation can be difficult.
:::
...
Floating-point encodings typically place the **sign**, **exponent**, and **significand field** from left to right. Use Wikipedia's [*Internal representation* section](https://en.wikipedia.org/wiki/Floating-point_arithmetic#Internal_representation) alongside this table to take notes on the difference between **storage bits** and **precision**.

| Format | Sign bits | Exponent bits | Stored significand bits |  Exponent bias $b$ | Precision (bits) | Approx. decimal digits |
|---|---:|---:|---:|---:|---:|---:|---:|
| Half (`float16` / binary16) | 1 | 5 | 10 |  15 | 11 | **3.3** |
| Single (`float32` / binary32) | 1 | 8 | 23 |  127 | 24 | **7.2** |
| Double (`float64` / binary64) | 1 | 11 | 52 | 1023 | 53 | **15.9** |

For normal values in these formats, the leading 1 is implicit: the stored significand field contains only the $t$ fraction bits, giving $t+1$ bits of precision.

The decimal-digit estimates are the bit precision multiplied by $\log_{10}2$; they are not guarantees of correct digits in a calculation. For comparison, the 128-bit and 256-bit binary formats provide approximately **34.0** and **71.3 significant decimal digits**, respectively; their availability depends on platform and software support.

The exponent uses a **bias**, not two's complement. Floats also use a separate sign bit rather than the two's-complement integer encoding above.

As you take notes, check that the three stored fields sum to the total width, and explain why `float64` has 52 stored fraction bits but 53 bits of precision.
...
::: {.callout-note collapse="true"}
## Polygon analysis: why the recurrence loses accuracy
...
:::: {.callout-note collapse="true"}
## Polygon experiment: numerical results

This table reports results for the polygon recurrence, not floating-point format specifications. Here $k$ counts side doublings, so $N=6\times2^k$.
