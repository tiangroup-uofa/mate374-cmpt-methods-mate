...
![Two rearrangements produce an approaching and a departing cobweb; slope ±1 cones compare each update's distance to a fixed point.](../../../assets/L06-fixed-point.png){fig-alt="Side-by-side cobweb diagrams for convergent and divergent rearrangements, with local cones bounded by slopes plus and minus one at the fixed points."}
...
Near the fixed point, a negative slope produces alternating errors; a positive slope preserves their sign.

The demo draws **slope ±1 cones** centred at the marked fixed points $(x_s,x_s)$. Their boundaries are $y=x_s\pm(x-x_s)$. If $(x,g(x))$ lies strictly inside a cone, then $|g(x)-x_s|<|x-x_s|$: that update brings the iterate closer to its tip. Compare the curve with these boundaries **near the fixed point**. The displayed cone width is chosen for visibility; it does not identify an interval of guaranteed convergence.
...
The cone compares $g(x)$ with one fixed point. The interval-wide Lipschitz bound above compares **every pair** of inputs. A plot can illustrate this geometry; proving the bound and interval invariance requires more than visual inspection. Also, $|g'(x_s)|<1$ is sufficient for local attraction, but is not necessary for every possible convergent iteration.
:::
