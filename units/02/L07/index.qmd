...
3. **Compare** bounded minimization, Newton iteration, and gradient descent by their information requirements, including numerical derivative estimates, and their failure modes.
...
For a one-component material at a specified temperature $T$ and pressure $P$, which state is most stable: liquid or gas?

In L05 and L06, the van der Waals equation gave us **possible volumes**. At some conditions, we found a liquid-like root and a gas-like root, with a mechanically unstable root between them. To choose between the two candidates, compare their **molar Gibbs free energies** at the same $T$ and $P$.

We now use **molar volume** $v=V/n$. Its units are volume per mole; the numerical values in L05's one-mole example stay the same when expressed in L/mol.

The free-energy landscape for a homogeneous vdW fluid at imposed $T$ and $P$ is
...
Here $v_{\mathrm{ref}}$ is a fixed reference volume that makes the logarithm dimensionless. $C(T)$ is common to the states being compared, so it cancels from their free-energy difference. This expression retains the dimensional vdW parameters and gives energy **per mole**.
...
![At 280 K and about 55.129 bar, the same stationary volumes appear as pressure intersections and free-energy extrema. The two minima have equal free energy.](../../../assets/L07-vdw-energy.png){#fig-vdw-energy fig-alt="Side-by-side dimensional CO2 van der Waals pressure and free-energy curves, with the two minima and intervening maximum marked at corresponding volumes." width="100%"}

The numerical task now has three parts:
...
Our focus is on smooth **interior** extrema. A finite search interval is a numerical aid. If the answer lies on its endpoint, or the physical problem imposes constraints, we must check those boundaries separately.

### Where are the stationary points?

At a smooth interior minimum or maximum, the tangent is horizontal:
...
For the curvature used by Newton's method, the same three function evaluations give
...
Suppose $f$ has one minimum in $[a,b]$, decreasing toward it and increasing after it. Such an interval is **unimodal**. We can compare function values inside the interval to discard a part that cannot contain the minimum.

A single midpoint value does not tell us which side has the lower values. Golden-section search evaluates **two interior points**, $x_1<x_2$:
...
![Golden-section search compares two interior values, discards one end, and reuses one evaluated point.](../../../assets/L07-golden-section.png){#fig-golden-section fig-alt="Two stages of a golden-section search showing a discarded left interval and reuse of the old right interior point as the new left point." width="100%"}
...
### Newton: find a zero of the slope
...
This needs the slope **and** curvature. Both can be supplied analytically or estimated from function values as above. Near a well-behaved minimum it can converge quickly, but it can also approach a maximum. Its root-finding target is a horizontal tangent.

For example, SciPy can solve the stationary-point equation directly:
...
Near a minimum with positive curvature, local attraction requires
...
For a quadratic well this gives the exact constant-step convergence range. For a general function it is a local test; the curvature changes as we move. Practical optimizers often adjust the step using a line search.
:::
...
The demo converts to SI units when calculating free energy. Its axes show molar volume in **L/mol**, pressure in **bar**, and free energy in **J/mol**. It uses $v_{\mathrm{ref}}=1$ L/mol and sets the common $C(T)$ to zero.

**Predict:** as pressure rises at fixed temperature, which minimum should become lower? Look at the $Pv$ term before moving the pressure slider.

::: {.quarto-wasm-local notebook="l07_co2_free_energy.edit.py" view="app" title="Compare dimensional CO2 free-energy minima and the van der Waals phase boundary" height="900" loading="lazy" fullscreen="true"}
![CO2 pressure, molar free energy, and the model liquid–gas boundary at 280 K and 50 bar.](../../../assets/L07-co2-demo.png){fig-alt="Pressure intersections and two free-energy wells alongside the calculated van der Waals liquid–gas coexistence line."}

**Static example.** At 280 K and 50 bar, the liquid-like and gas-like minima occur near 0.0876734 and 0.3065501 L/mol. Their free energies differ by $G_{\mathrm{liq}}-G_{\mathrm{gas}}\approx+98.10$ J/mol, so the gas-like state is favoured. Newton started at 0.11 L/mol reaches the intervening **maximum** near 0.1140563 L/mol. For this parameter set, the equal-minimum pressure at 280 K is about 55.129 bar.
:::

1. At **280 K and 50 bar**, locate the two minima. Which is lower?
2. Raise pressure to **56 bar**. Has the preferred phase changed?
3. Switch to Newton and start at **0.11 L/mol** at 280 K and 50 bar. The solver converges: did it find a minimum?
...
The bounded demonstration searches one well at a time, using the middle stationary volume to separate the wells when there are three roots. An independent cubic solve provides the stationary-volume reference. Newton uses an explicit update and reports steps that leave $v>b$.
...
Repeat at several temperatures to trace the liquid–gas phase boundary. The demo's right-hand plot shows this boundary for the dimensional vdW CO₂ model. The two controls move the chosen $(T,P)$ point across it.
