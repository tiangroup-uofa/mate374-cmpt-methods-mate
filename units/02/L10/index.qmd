...
The calculation needs four of our Unit 02 methods, each answering a different question. Read the diagram from left to right. Each box is one step of the program, and the steps in the shaded box repeat for every trial pressure until $\Delta G=0$.

```{mermaid}
flowchart LR
    B["1 · Fit a, b to gas data<br/>curve_fit"] --> L
    subgraph L["repeat at each T"]
        direction LR
        C["2 · Two wells<br/>minimize_scalar"] <--> D["3 · ΔG = 0<br/>root_scalar"]
    end
    L --> E["4 · Boundary<br/>PchipInterpolator"]
    style L fill:#fff8e1,stroke:#b08900,color:#5c4700
```
