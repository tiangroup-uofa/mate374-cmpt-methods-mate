...
1. **Fit** the two van der Waals parameters to measured pressure–volume–temperature data and assess the pressure residuals.
...
We fit **all 24 measurements with $\rho<10$ mol/L**. The figure shows the full set of 36 measurements, including the denser states where the vdW approximation becomes less accurate.

![Measured CO₂ pressure against molar density at five temperatures.](../../../assets/L10-co2-measurements.png){fig-alt="CO2 pressure–density measurements from 300 to 330 K, with one colour per temperature and the same marker style for every measurement." width="90%"}
...
We will write functions in the form $f(\mathrm{var}_1,\mathrm{var}_2;\mathrm{param}_1,\mathrm{param}_2)$. The semicolon separates the variables from the parameters held fixed when evaluating the function. For example, $P(T,v;a,b)$ takes temperature and molar volume as variables for a chosen pair of model parameters $a,b$. Which quantities vary depends on the numerical task. In Python, all arguments are separated by commas.
:::
...
Each state supplies two inputs, $T_i$ and $v_i$, and a measured output, $P_i$. The model predicts pressure. We fit one pair of parameters, $a$ and $b$, shared by all 24 states.
:::
...
Here $v_{\mathrm{ref}}=1\ \mathrm{L/mol}$ makes the logarithm dimensionless. The expression gives energy in L bar/mol, which we multiply by 100 to obtain J/mol. At a suitable subcritical temperature and pressure, this curve has two local minima: a liquid-like state at small molar volume and a gas-like state at large molar volume. We find each minimum in its own volume interval. The L07 figure below shows how the relative depths of these two wells change with pressure.

![The vdW isotherm and corresponding free-energy curves from L07, using the course parameters at 280 K. The gas well is lower at 50 bar, the wells are equal at 55.129 bar, and the liquid well is lower at 57.5 bar.](../../../assets/L07-vdw-energy.png){fig-alt="A vdW pressure–volume curve beside three free-energy curves with liquid-like and gas-like minima. Changing pressure changes their relative depths." width="100%"}
...
The L08 figure illustrates the same question: how should we connect a few calculated coexistence pressures to estimate the boundary between them?

![The phase-boundary interpolation problem from L08, illustrated with six coexistence calculations using the course vdW parameters.](../../../assets/L08-phase-points.png){fig-alt="Six saturation-pressure points between 250 and 295 K, with an arrow asking where the liquid–gas boundary lies between points." width="80%"}
...
+\bigl[P_{\mathrm{vdW}}(T_{24},v_{24};a,b)-P_{24}\bigr]^2\\
&=\sum_{i=1}^{24}\bigl[P_{\mathrm{vdW}}(T_i,v_i;a,b)-P_i\bigr]^2.
\end{aligned}
$$
...
$$
a=3.58867\ \mathrm{L^2\,bar/mol^2},\qquad b=0.042212\ \mathrm{L/mol}.
$$

### How well does the model fit?

Compare the predicted pressure $P_{\mathrm{vdW}}(T_i,v_i)$ with $P_i$ at the **24 fitting states**. The root-mean-square pressure residual is about **1.80 bar**. If we also evaluate the model at the twelve measurements above 10 mol/L, the residual for those denser states is about **129 bar**. The model captures the low-density trend but performs poorly at high density.
...
**Sanity check:** At $T=275$ K, using the fitted $a$ and $b$, you should obtain $P_{\mathrm{sat}}\approx50.04$ bar. Check that $\Delta G$ is close to zero at this pressure and that both phase volumes reproduce it through the EOS.
...
At 275 K, which lies between the five sample temperatures, compare `boundary(275.0)` with a fresh `solve_pressure(275.0, a_fit, b_fit)`. The direct calculation gives about **50.0448 bar**. Linear interpolation gives **50.1253 bar**, while PCHIP gives **50.0428 bar**, a difference from the direct calculation of about 0.002 bar. Repeat the comparison with linear and cubic interpolation to see how much the method matters.

## Answering the engineering question

Our fit gives $a\approx3.58867$ and $b\approx0.042212$, close to the course values $a=3.592$ and $b=0.04267$. A common way to determine vdW parameters is to match the critical temperature and pressure. Here we fitted pressure–volume–temperature data instead, yet the resulting model critical point,

$$
T_c=\frac{8a}{27Rb}\approx303.0\ \mathrm{K},
\qquad P_c=\frac{a}{27b^2}\approx74.6\ \mathrm{bar},
$$
...
|---:|---:|---:|---:|---:|
| 250 | 32.95 | 33.75 | 17.85 | +85% |
| 260 | 39.26 | 40.16 | 24.19 | +62% |
| 270 | 46.27 | 47.28 | 32.03 | +44% |
| 280 | 54.00 | 55.13 | 41.61 | +30% |
| 290 | 62.48 | 63.72 | 53.18 | +17% |

![CO₂ liquid–gas boundary predicted from gas-phase data. The red curve uses linear interpolation through five fitted-vdW coexistence calculations, and the dashed curve uses the course parameters. The black curve is the CO₂ reference. Open squares mark the model critical points, and blue crosses mark the 24 fitting states.](../../../assets/L10-phase-boundary.png){fig-alt="Pressure against temperature. Both vdW boundaries lie above the CO2 reference curve, with a larger relative discrepancy at lower temperatures. Their critical points lie close to the measured critical point." width="100%"}
...
[Open the reference notebook](../../../wasm-local/l10_phase_diagram_solution.html){target="_blank"} · [Download the reference notebook](../../../wasm-local/l10_phase_diagram_solution.edit.py){download="l10_phase_diagram_solution.edit.py"} · [Download the CO₂ data (CSV)](../../../data/L10-co2-pvt.csv){download="L10-co2-pvt.csv"}
...
The next unit is all about this equation.
